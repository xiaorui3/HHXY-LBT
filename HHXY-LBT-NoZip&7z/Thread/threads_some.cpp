...
//    VideoCapture cap("/home/shayu/桌面/欣竹视觉_步兵B/BamThriving-VISION/build/Video60.avi");
    ...
    int cameraID = 0;  // 相机ID
    bool cameraOpened = false;  // 相机是否打开的标志
	if(0){//是否执行opencv
	
    while (!cameraOpened && cameraID < 10)  // 如果相机没打开并且还有可用的ID
    {
        VideoCapture capture(cameraID);  // 打开相机
        printf("检测ID%d\n",cameraID);

        if (capture.isOpened())  // 如果相机打开了
        {
            cameraOpened=true;
            printf("检测成功ID : %d\n",cameraID);
            break;
